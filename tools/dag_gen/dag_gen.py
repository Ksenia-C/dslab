...
def compute_weights(graph, mindata, maxdata, ccr, scatter_gather=False):
    ...
    for node in graph:
        ...
        graph._node[node]["weight"] = input_size / ccr
...
def main():
    ...
    parser.add_argument("--mindata", type=float, nargs="*", default=[10],
                        help="min task input size in bytes")
    parser.add_argument("--maxdata", type=float, nargs="*", default=[100],
                        help="max task input size in bytes")
    parser.add_argument("--ccr", type=float, nargs="*", default=[1000.],
